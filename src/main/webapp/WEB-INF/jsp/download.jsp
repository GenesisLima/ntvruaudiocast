    ...
    <table>
        ...
        <c:forEach var="show" items="${shows}" varStatus="i">
            ...
            <tr bgcolor="${i.count % 2 == 0 ? '#f0f8ff' : '#f5f5dc'}">
                <td>
                    <a href="<c:url value="/downloads/files/${show.fileDocument.id}"/>" >
                        <c:choose>
                            ...
                            <c:when test="${show.fileDocument.fileType eq 'application/octet-stream'}">
                                <img src="<c:url value="/image/rar.jpg"/>" title="Download - ${show.fileDocument.fileName}"/>
                            </c:when>
                            <c:when test="${show.fileDocument.fileType eq 'application/msword'}">
                ...
                <td>${show.fileDocument.fileName}</td>
                <td>${show.fileDocument.fileType}</td>
                <td>
<%--                     <fmt:formatNumber var="value" value="${file.fileSize / 1024}" maxFractionDigits="2"/> --%>
                        ${value} KB
                </td>
                <td>${show.fileDocument.filePath}</td>
                <td><a href="<c:url value="/delete/file/${show.fileDocument.id}"/>">&cross;</a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
